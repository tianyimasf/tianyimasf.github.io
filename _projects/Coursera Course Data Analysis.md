...
This seems extremely true, because when I apply filters sequentially I got more result.
...
Since I'm already clicking into the course page, I added more fields into the dataframe, including skills you'll gain for a course. The problem is, some of these sections have a lot of tags, so just using beautifulsoup isn't enough if I want to get accurate information.
...
Using beautifulsoup, we're garanteed to get the latest source code, while selenium could end up getting any version of the older source code that is unpredictable and doesn't match our tag or classname element search.
...
I tried it out, and it works most of the time. I ended up looping until the ones that didn't work initially works on multiple tries.
